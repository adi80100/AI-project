    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      ...
      ${ isUser ? "bg-gradient-to-br from-indigo-500 to-violet-700 text-white rounded-tr-sm" :"text-slate-200 rounded-tl-sm" }`}>
             ...
             components={{
              ...
              td:({children})=>(
                <td className='border border-white/10 bg-white/5 px-3 py-2 text-center'>
                  {children}
                </td>
              ),
              a:({href,children})=>(
              ...
              code:({className,children})=>{
                ...
                if(!className) {
                  return (
                    <code className='px-1.5 py-0.5 rounded bg-white/10 text-green-500'>
                      {value}
                    </code>
                  )
                }
      ...
      <div className='fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6'>
        <button
        className='absolute top-5 right-5  text-white/80 hover:text-white bg-white/10 rounded-3xl p-4 text-2xl'
