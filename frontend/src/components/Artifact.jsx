  ...
  return (
   ...
   className='hidden lg:flex h-full border-l border-white/[0.06] flex-col overflow-hidden shrink-0 w-[250px]'>
       {!collapsed ? <div className='flex flex-col h-full bg-[#0d0f14]'>
            <div className='h-14 px-4 border border-white/[0.06] flex items-center gap-3 shrink-0'>
                ...
                <div className='flex items-center gap-2 flex-1 min-w-0'> 
                      ...
                      <div className='text-[13px] font-medium text-slate-200 truncate'>
                        {artifacts?.[0]?.title ?? 'No artifacts'}
                      </div>
                </div>
                ...
                {canPreview &&
                <div className='flex items-center cursor-pointer gap-1 bg-white/[0.04] border border-white/[0.06] p-1 rounded-lg'>
                <button
                  onClick={()=>setTab("code")}
                  className={`flex items-center cursor-pointer gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors duration-150
                ...
                <button
                  ...
                  className={`flex items-center cursor-pointer gap-1.5 px-2.5 py-1 text-[11px] font-medium rounded-md transition-colors duration-150
                ${ tab === "preview" ? " bg-indigo-500 text-white":
            ...
            {tab==="code" && <div className='h-auto flex border-b border-white/[0.06] overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden shrink-0'>
                   {/* artifacts is big array so here we have to use[0] here in order to see it */}
                    {artifacts[0]?.files?.map((f,idx)=>(

                       <button
                       onClick={()=>setActiveFile(idx)}
                        className={`px-4 py-2.5 text-[11px] font-medium whitespace-nowrap 
                        transition-colors duration-150 border-r border-white/[0.05] 
                        relative cursor-pointer bg-transparent text-indigo-400
                        ${activeFile===idx ? "text-indigo-400":"text-slate-500 hover:text-slate-300"}
                        `}
                       >
                        {f?.name}
                        {activeFile===idx &&
            ...
            <div className='flex-1 overflow-hidden'> 
              {(tab=="preview" && canPreview) ?
                ...
                >
                  <iframe srcDoc={prevDoc} sandbox='allow-scripts' title='preview' className='w-full h-full bg-white'></iframe>
                </motion.div> :
        ...
        <div className='hidden lg:flex  h-full border-1 border-white/[0.06]  bg-[#0d0f14] flex-col items-center py-3 gap-3 shrink-0'>
            <button 
                onClick={()=>setCollapsed(prev=>!prev)}
                className='flex items-center justify-center w-7 h-7 rounded-lg text-slate-500 hover:text-slate-200  hover:bg-white/[0.05] transition-colors duration-150 bg-transparent border-none cursor-pointer shrink-0'>
                  <PanelRightOpen/>
